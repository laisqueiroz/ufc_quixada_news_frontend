...
(async () => {
  ...
  try {
    ...
    console.log(
      "Playwright not installed, please install with: npm i -D playwright",
    );
    process.exit(1);
  }
  ...
  await page.waitForSelector("text=/disponível|indisponível/i", {
    timeout: 5000,
  });
  ...
  const [newPage] = await Promise.all([
    ...
    // support both editor states (editing & creating)
    page.click(
      'button:has-text("Pré-visualizar"), button:has-text("Salvar Rascunho e Pré Visualizar")',
    ),
  ]);
