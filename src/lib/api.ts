...
export type Papel = 'ESTUDANTE' | 'DOCENTE' | 'SERVIDOR' | 'BOLSISTA' | 'ADMINISTRADOR'
export type Categoria = 'EVENTOS' | 'OPORTUNIDADES' | 'PESQUISA' | 'PROJETOS' | 'AVISOS' | 'OUTROS'
export type TipoReacao = 'CURTIDA' | 'AMEI' | 'TRISTE' | 'SURPRESO' | 'PARABENS'
...
export class ApiClient {
    ...
    async getReactions(articleId: number): Promise<Record<TipoReacao, number>> {
        return this.request<Record<TipoReacao, number>>(`/news/${articleId}/reacoes`)
    }
    ...
    async deleteReaction(articleId: number): Promise<{ ok: boolean }> {
        return this.request<{ ok: boolean }>(`/news/${articleId}/reacoes`, { method: 'DELETE' })
    }

    async deleteReactionById(articleId: number, reacaoId: number): Promise<{ ok: boolean }> {
        return this.request<{ ok: boolean }>(`/news/${articleId}/reacoes/${reacaoId}`, {
            method: 'DELETE',
        })
    }
    ...
    async getMyCommentReaction(
        ...
        const res = await this.request<any>(`/news/${articleId}/comentarios/${comentarioId}/reacao`)
        if (res === null) return { id: null, tipo: null }
    ...
    async createCommentReaction(
        ...
        await this.request(`/news/${articleId}/comentarios/${comentarioId}/reacoes`, {
            method: 'POST',
            body: JSON.stringify({ tipo }),
        })
    }

    async deleteCommentReaction(articleId: number, comentarioId: number): Promise<{ ok: boolean }> {
    ...
    async getManageArticles(params?: {
        ...
        const query = searchParams.toString()
        return this.request<ArtigoListResponse>(`/gerenciar/artigos${query ? `?${query}` : ''}`)
    }
    ...
    async checkSlugAvailability(slug: string, ignoreId?: number): Promise<boolean> {
        const params = new URLSearchParams()
        params.append('slug', slug)
        if (typeof ignoreId === 'number') params.append('ignoreId', String(ignoreId))
    ...
    async createSolicitacao(tipo: TipoSolicitacao, mensagem?: string): Promise<Solicitacao> {
        return this.request<Solicitacao>('/solicitacoes', {
            method: 'POST',
