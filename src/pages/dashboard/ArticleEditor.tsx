...
import { ArrowLeft, Plus, Trash2, Loader2, Save, Eye, GripVertical, Image } from 'lucide-react'
import {
    DndContext,
    PointerSensor,
    useSensor,
...
export default function ArticleEditor() {
    ...
    const updateSessao = (uid: string, updates: Partial<LocalSessao>) => {
        setForm((prev) => ({
            ...prev,
            sessoes: prev.sessoes.map((s) => (s._uid === uid ? { ...s, ...updates } : s)),
        }))
    }
    ...
    const removeSessao = (uid: string) => {
        setForm((prev) => ({
            ...prev,
            sessoes: prev.sessoes.filter((s) => s._uid !== uid).map((s, i) => ({ ...s, ordem: i })),
        }))
    }

    const handleUpdateSection = (index: number, updates: Partial<ArtigoSessao>) => {
        setForm((prev) => ({
            ...prev,
            sessoes: prev.sessoes.map((s, i) => (i === index ? { ...s, ...updates } : s)),
        }))
    }
    ...
    const handleRemoveSection = (index: number) => {
        setForm((prev) => ({
            ...prev,
            sessoes: prev.sessoes.filter((_, i) => i !== index).map((s, i) => ({ ...s, ordem: i })),
        }))
    }
    ...
    const handleSubmit = async (publish: boolean = form.publicado) => {
        if (!form.titulo || form.titulo.length < 3) {
            toast({ title: 'O título deve ter pelo menos 3 caracteres', variant: 'destructive' })
            return
        }
        ...
        if (
            ...
            toast({
                title: 'Adicione ao menos uma seção com texto ou imagem antes de publicar',
                variant: 'destructive',
            })
            return
        }
                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>
                                Adicione parágrafos, tópicos ou imagens para enriquecer o artigo.
                            </CardDescription>
                        </div>
                ...
                <CardContent className="pt-6">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div className="flex gap-3">
                            <Button
                                variant="outline"
                                onClick={() => handleSubmit(isEditing ? form.publicado : false)}
                                disabled={isSaving}
                            >
                                {isSaving ? (
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    ...
                                    if (!form.titulo || form.titulo.length < 3) {
                                        toast({
                                            title: 'Salve o artigo antes de pré-visualizar (título curto)',
                                            variant: 'destructive',
                                        })
                                        return
                                    }
                                    ...
                                    if (form.slug && slugAvailable === false) {
                                        toast({
                                            title: 'Slug em uso. Escolha um slug diferente antes de pré-visualizar',
                                            variant: 'destructive',
                                        })
                                        return
                                    }
