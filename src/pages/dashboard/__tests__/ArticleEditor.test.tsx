...
describe("ArticleEditor action buttons (create vs edit)", () => {
  ...
  describe("ArticleEditor image session (URL and upload)", () => {
    beforeEach(() => vi.resetAllMocks());

    test("allows entering image URL and sends it when creating", async () => {
      const createSpy = vi
      ...
      // URL input should be present by default
      const urlInput = screen.getByPlaceholderText(/URL da imagem/i);
      fireEvent.change(urlInput, { target: { value: "https://cdn/test.png" } });
      // blur to flush debounced update
      fireEvent.blur(urlInput);

      // fill required title and publish
      fireEvent.change(screen.getByPlaceholderText(/Digite o título/i), {
        target: { value: "T" + "itulo" },
      });
      ...
      await waitFor(() => {
        ...
        expect(payload.artigoSessoes[0].imagemUrl).toBe("https://cdn/test.png");
      });
    });
